...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='logs/context_sig8_mag2_up2')
    ...
    train_dataset = lib.dataset_context.OMCDataset(args.train_h5_path, args.image_size, args.target_size, args.sigma, args.magnitude)
    ...
    for epoch_i in epoch_iter:
        ...
        model.train()
        for batch_i, batch in tqdm.tqdm(enumerate(train_dataloader), total=len(train_dataloader), leave=False):
        ...
        with torch.no_grad():
            if epoch_i % args.f_val == 0:
                ...
                metrics = collections.defaultdict(list)
                model.eval()
                for batch_i, batch in tqdm.tqdm(enumerate(val_dataloader), total=len(val_dataloader),
                                                                leave=False):
                    image, target, bbox, cls = [x.to(args.device) for x in batch]
                    bbox = bbox.cpu()
                    if args.context:
