    ...
    with torch.no_grad():
        for image_i, (image, target, bbox) in image_iter:
            ...
            for landmark_i in range(17):
                plt.gcf().gca().add_patch(plt.Circle((prediction_landmarks[landmark_i][0] - bbox[0], prediction_landmarks[landmark_i][1] - bbox[1]), color='r', radius=2))
                plt.gcf().gca().add_patch(plt.Circle((target_landmarks[landmark_i][0], target_landmarks[landmark_i][1]), radius=2, color='b'))
                plt.gcf().gca().add_patch(plt.Line2D((target_landmarks[landmark_i][0], prediction_landmarks[landmark_i][0] - bbox[0]),
                                                     (target_landmarks[landmark_i][1], prediction_landmarks[landmark_i][1] - bbox[1]),))
